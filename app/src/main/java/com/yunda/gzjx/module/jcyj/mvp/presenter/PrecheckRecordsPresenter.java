 ...
 */
@ActivityScope
public class PrecheckRecordsPresenter extends BasePresenter<PrecheckRecordsContract.Model, PrecheckRecordsContract.View> {
    @Inject
    ...
    @Inject
    public PrecheckRecordsPresenter(PrecheckRecordsContract.Model model, PrecheckRecordsContract.View rootView) {
    ...
    public void searchJXTasksOfProjectOnLocal(List<JXTask> source, String keyWord) {
        ...
        for (JXTask jxTask : source) {
            if (jxTask.workTaskName.contains(keyWord)) {
                if (resSearched == null) {
